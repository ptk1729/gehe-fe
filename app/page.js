    ...
    return (
        <>
            <main className='flex flex-col items-center min-h-screen bg-gray-100 p-4'>
                ...
                <form onSubmit={handleShortenUrl} className='w-full max-w-md'>
                    <div className='flex items-center border-b border-teal-500 py-2'>
                        ...
                        <button
                            type='submit'
                            disabled={loading}
                            className={`flex-shrink-0 bg-teal-500 hover:bg-teal-700 border-teal-500 hover:border-teal-700 text-sm border-4 text-white py-1 px-2 rounded ${loading ? "opacity-50 cursor-not-allowed" : ""
                                }`}
                        >
                ...
                {URLs.length > 0 &&
                    URLs.sort((a, b) => new Date(a.updatedAt).getTime() - new Date(b.updatedAt).getTime()).map(
                        ({
                            archived,
                            clicks,
                            createdAt,
                            ...
                            <div key={id} className='mt-8 p-4 bg-white rounded shadow-md text-center'>
                                ...
                                <p className="text-sm"><TimeAgo date={updatedAt} /></p>
